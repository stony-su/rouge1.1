...
function BallPit:reset_run()
  ...
  self.powerup_pity = {
    ...
    tier2_chance   = BAL('powerups.tier2_chance', 0.20),   -- on a spawn, this fraction roll the rare tier
  }
...
function BallPit:update(dt)
  ...
  -- consumers.
  if input.toggle_terminal.pressed and not self.controls_open then self.terminal:toggle() end
  if input.toggle_dmg_tally.pressed and not self.controls_open then self.show_dmg_tally = not self.show_dmg_tally end
  if input.click.pressed and self:admin_button_hit() and not self.controls_open then
  ...
  -- Input-driven, so it fires once per real frame (not per speed sub-step).
  ...
  -- Input-driven, so it also fires once per real frame, not per sub-step.
  ...
  for step = 1, steps do
    ...
    if (self.fire_lock_t or 0) > 0 then self.fire_lock_t = self.fire_lock_t - sdt end
    ...
    -- sub-step dt so the admin speed cheat scales it like everything else.
    if self.run_mods and self.run_mods.hp_mode == 'bar' then
      self.player_hp = self.player_hp - (self.run_mods.sig.drain or 2)*sdt
...
-- Damage-source tally sink (admin balance overlay). Every enemy take_damage
-- (Brick / EnemyCritter / Boss) reports its post-multiplier, overkill-clamped
-- amount here with a source tag: a hero character name, a DoT channel
-- ('burn' / 'bleed' / 'infest'), or a signature/mechanism name ('tesla',
-- 'detonate', 'fusion', 'maggot', 'parry', 'exploder'). Untagged damage paths
-- land in 'other', so the total is exact even when a tag is missed.
function BallPit:track_damage(source, amount)
  if not amount or amount <= 0 then return end
  self.dmg_tally = self.dmg_tally or {}
  source = source or 'other'
  self.dmg_tally[source] = (self.dmg_tally[source] or 0) + amount
  self.dmg_tally_total   = (self.dmg_tally_total or 0) + amount
end


-- Admin damage-source overlay (F2 / terminal `dmg`): a small translucent
-- panel in the playfield's top-right corner listing every damage source this
-- run with its total and share of all damage dealt. Fed by track_damage.
function BallPit:draw_dmg_tally()
  local total = self.dmg_tally_total or 0
  local rows  = {}
  for src, amt in pairs(self.dmg_tally or {}) do rows[#rows + 1] = {src = src, amt = amt} end
  table.sort(rows, function(a, b) return a.amt > b.amt end)
  local max_rows = 12
  local shown    = math.min(#rows, max_rows)
  local extra    = #rows - shown
  ...
  local pad    = 5
  local w      = 124
  local h      = pad*2 + line_h*(1 + math.max(shown, 1) + (extra > 0 and 1 or 0))
  local right  = self.x2 - 4        -- just inside the arena's right wall
  local top    = self.y1 + 4

  graphics.rectangle(right - w/2, top + h/2, w, h, 3, 3, Color(0, 0, 0, 0.55))
  graphics.rectangle(right - w/2, top + h/2, w, h, 3, 3, fg_transparent_weak, 1)

  local lx = right - w + pad
  local y  = top + pad - 1
  graphics.print('DMG', pixul_font, lx, y, 0, 1, 1, 0, 0, yellow[0])
  local tot_s = fmt(total)
  graphics.print(tot_s, pixul_font, right - pad - pixul_font:get_text_width(tot_s), y, 0, 1, 1, 0, 0, yellow[0])
  y = y + line_h

  if shown == 0 then
    graphics.print('no damage yet', pixul_font, lx, y, 0, 1, 1, 0, 0, fg_alt[0])
    return
  end
  for i = 1, shown do
    local r     = rows[i]
    local color = character_colors[r.src] or fg[0]
    local pct   = (total > 0) and (100*r.amt/total) or 0
    local rs    = fmt(r.amt) .. ' ' .. string.format('%2.0f%%', pct)
    graphics.print(tostring(r.src):sub(1, 10), pixul_font, lx, y, 0, 1, 1, 0, 0, color)
    graphics.print(rs, pixul_font, right - pad - pixul_font:get_text_width(rs), y, 0, 1, 1, 0, 0, fg[0])
    y = y + line_h
  end
  if extra > 0 then
    graphics.print('+' .. extra .. ' more', pixul_font, lx, y, 0, 1, 1, 0, 0, fg_alt[0])
  end
end
...
function BallPit:do_chain_lightning(x, y, dmg, chain_len, color, source)
  local hit_ids = {}
  local cx, cy = x, y
  local hit_any = false
  for i = 1, chain_len do
    ...
    target:take_damage(dmg, color, nil, source)
...
function BallPit:apply_powerup(kind, x, y, color, amount)
  ...
  elseif kind == 'level_random' then
    self:apply_level_random(amount)
  end
end
...
function BallPit:apply_multi_ball()
  ...
  -- Wind the clones down after 12 s. Iterate the captured table, not
  -- self.heroes, so we only touch the clones we just spawned.
  --
  -- They used to be killed outright here -- setActive(false) + dead on the
  -- same tick -- so half a full field blinked out between two frames and read
  -- as a bug. Each copy now collapses over copy_out and removes ITSELF at the
  -- end (BallHero:copy_expire, which also compacts the roster).
