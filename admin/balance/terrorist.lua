-- ==========================================================================
-- BALL PIT X — DAMAGE MASTER FILE — Terrorist paddle
-- ==========================================================================
-- This file is the single source of truth for every damage number in the
-- game WHILE THE Terrorist PADDLE IS EQUIPPED. Each paddle has its own copy
...
return {
  ...
  loadout = {
    dmg = 1.6, size = 1.0, move = 1.0, ball = 1.1, charge = 1.0, aim = 1.0,
    xp = 1.0, combo = 1.0, hp = 3,
  },

  -- Manual detonation (E). The blast is the whole build; everything else
  -- is gutted by other_dmg_mult.
  signature = {
    blast_radius           = 78,
    blast_radius_per_level = 0.05,   -- +5% radius per run level
    blast_radius_max       = 150,
    blast_mult             = 5.0,    -- blast damage = this x ball damage
    blast_dmg_per_level    = 0.18,   -- +18% blast damage per run level
    other_dmg_mult         = 0.2,    -- ALL non-blast damage is scaled by this
    -- Element carry-over layered onto the blast:
    element_burn_dps_mult  = 0.35,   -- Boss only; bricks burn at dots.burn_max_hp_frac
    element_burn_duration  = 3,
    element_slow_factor    = 0.5,
    element_slow_duration  = 3,
  },
  ...
  -- Terrorist: untouched — the full demolition sight from level 1.
  aim = {
    path_base      = 1300,
    path_per_level = 0,
  },
