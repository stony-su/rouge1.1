-- ==========================================================================
-- BALL PIT X — DAMAGE MASTER FILE — Cannon paddle
-- ==========================================================================
-- This file is the single source of truth for every damage number in the
-- game WHILE THE Cannon PADDLE IS EQUIPPED. Each paddle has its own copy
...
return {
  ...
  loadout = {
    dmg = 1.5, size = 0.9, move = 1.0, ball = 0.6, charge = 1.7, aim = 0.9,
    xp = 1.0, combo = 1.1, hp = 4,
  },

  -- Paddle-strike hops: airborne balls crash down in falloff splashes.
  -- Damage lives in bounces + hop landings; abilities are damped.
  signature = {
    other_dmg_mult = 0.35,   -- ability/projectile/pet damage is scaled by this
    -- Hop physics + payoff (harder paddle strike = higher hop = bigger splash):
    hop_vz_min      = 120,   -- launch z-speed at a soft tap
    hop_vz_max      = 300,   -- launch z-speed at a full-charge strike
    hop_gravity     = 540,
    hop_restitution = 0.72,  -- z-speed kept per landing (shrinking hops)
    hop_vz_end      = 60,    -- below this rebound the hop settles flat
    hop_radius_min  = 22,    -- splash radius per landing (soft -> hard)
    hop_radius_max  = 58,
    hop_dmg_min     = 1.2,   -- x ball damage per landing (soft -> hard)
    hop_dmg_max     = 3.4,
  },
  ...
  -- Cannon: artillery rangefinding — long.
  aim = {
    path_base      = 180,
    path_per_level = 16,
  },
